...
function isObject() {
  ...
  function validate(value, onError) {
    ...
    for (var parameterName in rules) {
      ...
      if (parameterValue === undefined && rule.required) {
        onError('Required value.', parameterName, parameterValue);
        continue;
      }

      if (!parameterValue || !rule.validator) {
        continue;
      }
      ...
      function bubbleError(message, childName, childValue) {
        ...
        onError(message, name, childValue || parameterValue)
      }
  ...
  function validate(value, onError) {
    ...
    if (options) {
      if (options.regex && !options.regex.test(value)) {
        return onError('Invalid value. Value must match required pattern.');
      }
    }
    return null;
  }
...
function isDate(options) {
  ...
  function validate(value, onError) {
    ...
    if (typeof value !== 'string') {
      return onError('Incorrect type. Expected date string.');
    }
    if (!moment(value, options.format, true).isValid()) {
      return onError('Invalid value. Format is not valid.');
    }
    return null;
  }
}
...
function isArray(validator, options) {
  ...
  function validate(value, onError) {
    ...
    if (validator) {
      for (var j in value) {
        function bubbleError(message, childName, childValue) {
          onError(message, childName ? '[' + j + '].' + childName : '[' + j + ']', childValue || value[j])
        }
